...
function InstagramStrip({
  ...
  useBlockEffect(() => {
    ...
    let cancelled = false;
    fetch(`https://feeds.behold.so/${feedId}`).then(r => r.ok ? r.json() : Promise.reject(new Error(String(r.status)))).then(d => {
  ...
  const tiles = posts && posts.length ? posts.slice(0, 6).map(p => ({
    key: p.id,
    src: p.sizes && (p.sizes.medium && p.sizes.medium.mediaUrl || p.sizes.small && p.sizes.small.mediaUrl) || p.thumbnailUrl || p.mediaUrl,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "ig-grid"
  }, tiles.map(t => /*#__PURE__*/React.createElement("a", {
    key: t.key,
    className: "ig-tile",
    href: t.href,
    target: "_blank",
